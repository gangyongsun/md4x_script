...
class API_Com_K_Capc(object):
    ...
    def mainProcess(self, data, gua_pwr_i):
        # 功能：求该机组的各仓符合度、平均符合度、K值、功率曲线一致度（CAPC）
        ...
        windbin_com.to_csv(self.result_dir + '/' + str(data.loc[0, 'wtid']) + '_4_1_com_windbin.csv', index=False)
        com_k_capc.to_csv(self.result_dir + '/' + str(data.loc[0, 'wtid']) + '_4_1_com_k_capc.csv', index=False)
        return ()

    def dataProcess(self, gua_pwr, wtid_info):
        # 功能：求该机组的各仓符合度、平均符合度、K值、功率曲线一致度（CAPC）
        # 输入：
        #   gua_pwr——担保/设计曲线；
        #   wtid_info——海拔、空气密度
        ...
        self.mainProcess(data, gua_pwr_i)
        return ()
...
if __name__ == '__main__':
    ...
    instance = API_Com_K_Capc()
    instance.dataProcess(gua_pwr, wtid_info)
